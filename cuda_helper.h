...
/*
__forceinline__ __device__ uint64_t byte(uint64_t x,uint32_t n) 
{
uint64_t result;
unsigned res;
asm("shr.b64 %0,%1,%2;" : "=l"(result) : "l"(x),"r"(8*n)); 
res= (unsigned) result;
asm("and.u16 %0,%0,0xFF;" : "+h"(res));
return res;
}

__forceinline__ __device__ unsigned byte(uint64_t x,uint32_t n)
{
uint64_t result;
asm("{\n\t"
	".reg .u64 m;\n\t"
